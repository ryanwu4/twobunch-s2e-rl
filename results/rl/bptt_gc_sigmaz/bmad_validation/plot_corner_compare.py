...
Per-bunch centroids are marked so the inter-bunch transverse OFFSET is explicit, and the z-marginal
shows the bunch length (sigma_z) the new objective targets.

Reads results/rl/bptt_gc_sigmaz/bmad_validation/sample_<g>_PENT.h5 (Bmad) + results/rl/bptt_gc_sigmaz/setpoints/setpoints_goal<g>um.json
(the normalized knobs) + the flow checkpoint.

Usage: CUDA_VISIBLE_DEVICES=1 PYTHONPATH=$PWD/src MPLBACKEND=Agg \
    python results/rl/bptt_gc_sigmaz/bmad_validation/plot_corner_compare.py \
