...
Usage: CUDA_VISIBLE_DEVICES=1 PYTHONPATH=$PWD/src MPLBACKEND=Agg \
    python results/rl/bptt_gc_sigmaz/bmad_validation/plot_corner_sweep.py \
...
def lps_fig(A, B, cA, cB, labA, labB, title, lims):
    """z-pz scatter with marginal histograms (z along top, pz along right). Fixed bins from `lims`
    so the sweep GIF frames stay registered."""
    fig = plt.figure(figsize=(7.8, 6.6))
    gs = fig.add_gridspec(2, 2, width_ratios=(4.5, 1), height_ratios=(1, 4.5),
                          left=0.11, right=0.97, bottom=0.10, top=0.90, wspace=0.04, hspace=0.04)
    ax = fig.add_subplot(gs[1, 0])
    axtop = fig.add_subplot(gs[0, 0], sharex=ax)
    axright = fig.add_subplot(gs[1, 1], sharey=ax)
    zbins = np.linspace(*lims[2], 60)
    pbins = np.linspace(*lims[5], 60)
    for (d, w), col, lab in ((A, cA, labA), (B, cB, labB)):
        c = np.vstack([x for x in (d, w) if x is not None])
        ax.scatter(c[:, 2], c[:, 5], s=4, alpha=0.22, color=col, edgecolors="none", label=lab)
        axtop.hist(c[:, 2], bins=zbins, density=True, histtype="step", lw=1.4, color=col)
        axright.hist(c[:, 5], bins=pbins, density=True, histtype="step", lw=1.4, color=col,
                     orientation="horizontal")
    ax.set_xlim(*lims[2]); ax.set_ylim(*lims[5])
    ax.set_xlabel("z [mm]"); ax.set_ylabel("pz [GeV/c]")
    ax.legend(fontsize=10, loc="upper right")
    axtop.tick_params(labelsize=6, labelbottom=False); axtop.set_ylabel("density", fontsize=7)
    axright.tick_params(labelsize=6, labelleft=False); axright.set_xlabel("density", fontsize=7)
