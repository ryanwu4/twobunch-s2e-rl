...
Reads results/rl/bptt_gc_sigmaz/bmad_validation/validate_goal*um.json (from scripts/validate_bmad.py) and makes:
...
Includes the new drive/witness sigma_z (bunch length) metrics; ../sweep_analysis/sigma_z_vs_goal.png
gives the richer sigma_z view (surrogate vs Bmad vs golden vs target vs the pre-objective drift).

Usage: PYTHONPATH=$PWD/src MPLBACKEND=Agg python results/rl/bptt_gc_sigmaz/bmad_validation/plot_validation.py
"""
import glob
import json
...
METRICS = [
    ...
    ("witness_sigma_z_um", "witness σ_z [µm]", False, 10.2),      # golden witness bunch length
    ("drive_sigma_z_um", "drive σ_z [µm]", False, 27.3),          # golden drive bunch length
]
...
def main():
    ...
    ax.set_title("Surrogate vs Bmad — points far above y=x = optimistic surrogate\n"
                 "(○ = smallest target, △ = larger targets)", fontsize=11)
