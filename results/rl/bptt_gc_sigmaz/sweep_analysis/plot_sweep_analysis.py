"""Full-sweep (100-300 um) analysis for the sigma_z-matched goal-conditioned controller:
...
The sigma_z objective itself is validated separately in sigma_z_vs_goal.png (plot_sigma_z.py).

Usage: PYTHONPATH=$PWD/src MPLBACKEND=Agg python results/rl/bptt_gc_sigmaz/sweep_analysis/plot_sweep_analysis.py
"""
import json
...
def main():
    ...
    kmed = np.array([json.load(open(repo_root() / f"logs/{RUN}/eval_best_goal{g}um.json"))
    ...
    fig.suptitle("Surrogate vs Bmad across the spacing sweep (sigma_z-matched)", fontsize=13)
    ...
    a.set_title("Spacing tracking (sigma_z-matched)", fontsize=11)
    ...
    fig.suptitle("Goal-conditioned knob set-points across the spacing sweep (sigma_z-matched, all 26 knobs)",
                 fontsize=13, y=0.998)
